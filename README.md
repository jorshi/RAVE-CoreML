# RAVE - Core ML accelerated inference

This is a fork of [RAVE (Realtime Audio Variational autoEncoder) ](https://github.com/acids-ircam/RAVE). It adds support for exporting RAVEs to [ExecuTorch](https://docs.pytorch.org/executorch/) for use with [**`neural.live~`**](https://github.com/jasper-zheng/neural_tilde) for hardware-accelerated inference on Apple Silicon.
...
Export needs extra packages that are **not** installed by a plain `uv sync`, install them with:
...
This writes a **pair of files sharing a basename** as required by [`neural.live~`](https://github.com/jasper-zheng/neural_tilde):
...
- `--backend {coreml,xnnpack,portable}`: delegate to lower to (default `coreml`).
  `coreml` targets the Apple Neural Engine / GPU via Core ML; `xnnpack` is a optimised CPU backend; `portable` uses plain PyTorch.
...
> **Core ML runtime** requires **macOS 15+ / iOS 18 on Apple Silicon** (the Neural Engine) at inference time.
...
so it processes consecutive audio blocks without buffer-boundary clicks. 



## Belows are the official readme from [RAVE](https://github.com/acids-ircam/RAVE) acids-ircam
